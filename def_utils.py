...
def _invalidate_cache(ptr, size):
    """OCM 영역 cache invalidate"""
    libc = ctypes.CDLL("libc.so.6")
    # MADV_DONTNEED = 4
    libc.madvise(ctypes.c_void_p(ptr), ctypes.c_size_t(size), 4)
...
def wait_uio_irq(uio_index: int, timeout_ms: int = 5000) -> int:
    """
    /dev/uioX에서 인터럽트 1회 대기.
    - interrupt 발생 시: print + irq_count 반환
    - timeout 시      : print + 0 반환
    """
    ...
    if not events:
        ...
        print(f"[{now}] [TIMEOUT] UIO{uio_index} no interrupt")
    ...
    # ---------- INTERRUPT ----------
    data = os.read(fd, 4)
    irq_count = struct.unpack("I", data)[0]

    #now = time.strftime("%H:%M:%S")
    #print(f"[{now}] [INTERRUPT] UIO{uio_index} irq_count = {irq_count}")
